...
import { Mic, MicOff, VolumeX, X, Send, Sparkles, Bot, User, Settings, ChevronDown } from 'lucide-react';
import { AI_ASSISTANT_TOGGLE_EVENT } from './Navbar';
...
// 大模型API调用（示例，需要配置API密钥）
async function callLLMAPI(_question: string): Promise<string | null> {
  // 这里可以接入真实的LLM API，如OpenAI、文心一言、通义千问等
  // 示例代码（需要配置API密钥）：
  /*
  try {
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-3.5-turbo',
        messages: [
          { role: 'system', content: '你是霍普金森杆测试系统的AI助手，请基于专业知识回答问题。' },
...
export default function AIVoiceAssistant() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(AI_ASSISTANT_TOGGLE_EVENT, handleToggle);
    return () => window.removeEventListener(AI_ASSISTANT_TOGGLE_EVENT, handleToggle);
  }, []);
  ...
  const handleUserInput = async (text: string) => {
    ...
    try {
      ...
      // 如果本地没有，尝试调用大模型API
      if (!answer) {
        answer = await callLLMAPI(text);
      }
      
      // 如果都没有，返回默认回复
      if (!answer) {
        answer = '抱歉，我暂时无法回答这个问题。您可以尝试询问以下方面：\n• 电磁驱动霍普金森杆系统介绍\n• AI三级闭环算法\n• 材料测试方法\n• 虚拟实验室使用\n• 系统监控和预警';
      }
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 w-[450px] h-[600px] bg-[#0A2540]/95 backdrop-blur-xl rounded-2xl border border-[#00F5FF]/30 shadow-2xl z-50 flex flex-col overflow-hidden"
          >
            {/* 头部 */}
            <div className="flex items-center justify-between p-4 border-b border-[#00F5FF]/20 bg-gradient-to-r from-[#00F5FF]/10 to-transparent">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/50">专业知识问答</p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.length === 0 && (
                <div className="text-center text-white/40 py-8">
                  ...
                  <p className="mb-2 text-lg">👋 您好！我是您的AI技术助手</p>
                  <p className="text-sm">可以问我关于电磁驱动霍普金森杆的任何问题</p>
                  <div className="mt-6 space-y-2">
                    <p className="text-xs text-white/30 mb-3">示例问题：</p>
