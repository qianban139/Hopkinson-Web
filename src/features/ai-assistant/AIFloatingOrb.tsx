...
// AI悬浮球 - 72px + 旋转光环 + idle呼吸脉冲 + "小智"标签
// 与聊天面板共享 layoutId="ai-shell"，实现连续过渡动画
...
export default function AIFloatingOrb({ orbState, onTogglePanel, isPanelOpen, onPushToTalkStart, onPushToTalkEnd, isPushToTalk }: AIFloatingOrbProps) {
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    const newLeft = Math.max(0, Math.min(maxLeft, posStartRef.current.left + dx));
    const newTop = Math.max(0, Math.min(maxTop, posStartRef.current.top + dy));
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    ...
    if (isLongPressRef.current) {
      ...
      // 拖拽结束：从 DOM 读取最终位置同步到 state（触发 localStorage 保存）
      const dx = e.clientX - pointerStartRef.current.x;
      const dy = e.clientY - pointerStartRef.current.y;
      const maxLeft = window.innerWidth - 80;
      const maxTop = window.innerHeight - 100;
